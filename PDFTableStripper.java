...
public class PDFTableStripper extends PDFTextStripper
{
	...
	{
		try (PDDocument document = PDDocument.load(new File("C:/Users/jooeu/Downloads/CM-AFC2000plus.pdf")))
		{
			final double res = 20; // PDF units are at 72 DPI
			PDFTableStripper stripper = new PDFTableStripper();
			stripper.setSortByPosition(true);
			
			// Choose a region in which to extract a table (here a 6"wide, 9" high rectangle offset 1" from top left of page)
			stripper.setRegion(new Rectangle((int) Math.round(1.0*res), (int) Math.round(1*res), (int) Math.round(30*res), (int) Math.round(15.0*res)));
